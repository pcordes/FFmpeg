                          ...
                          AVFrame *cur, AVFrame *ref)
{
	 ...
	 * localized changes in a subsampled plane that wouldn't exceed the
	 * thresholds when diluted by using what's effectively a larger block size
	 */
...
/* We can handle any format where it makes sense to SAD bytes,
 * even if our 8x8 block boundaries split up components of a single pixel.
 * (with some loss in the precise meaning of high, low, and frac).
 * e.g. RGB24 is fine
 *
 * We would need different SAD routines to support formats where the LSB of
 * every byte isn't the LSB of a color component.  Otherwise we could ignore
 * big changes if they mostly happened in the LSB of the packed bytes
 * (false positive), or fail to find dups because of small component differences
 * being stored in the high bits of the packed bytes.
 * e.g. we can't handle AV_PIX_FMT_RGB48LE, AV_PIX_FMT_RGB565LE, or
 * AV_PIX_FMT_YUV420P10LE
 * Wide pix fmts probably need to be native endian.
 */


/* disable this to force conversion to formats where our 8x8 blocks contain 8x8 pixels.
  (or 8x16, or 16x16 pixels, or whatever for subsampled planes) */
#define DECIMATE_PACKED_HACK 1
